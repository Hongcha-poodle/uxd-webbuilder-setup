...
- 브라우저 스크린샷 캡처 수단이 확보된 상태여야 합니다 (MCP 브라우저 도구 또는 사용자 수동 제공).

## Workflow Steps

> 다중 타겟 처리 원칙: 타겟이 여러 개인 경우, 본 워크플로우는 컴포넌트별로 순차 실행합니다. 각 반복은 단일 컴포넌트만 대상으로 합니다.
...
2. **브라우저 렌더링 캡처**:
   ...
   - **URL 사전 접근 체크**: `http://localhost:3000/preview/raw/[ComponentName]` URL에 접근하여 정상 응답(200)을 확인합니다.
     - **404 반환 시**: 컴포넌트 파일명 대소문자 불일치 또는 파일 미존재를 진단하고, 원인을 리포트에 기록한 뒤 워크플로우를 중단합니다.
     - **서버 미응답 시**: "개발 서버가 실행 중이지 않습니다." 안내 후 워크플로우를 중단합니다.
   - URL 접근이 정상이면 스크린샷을 캡처합니다.
...
- **반복 조건**: `수정 대상 이슈 남아있음` AND `iteration < 3` AND `consecutive_rollbacks < 2`
- **매 반복(iteration)마다**:
...
- URL 사전 접근 체크에서 404가 반환된 경우 (파일명 불일치 진단 결과 포함)
