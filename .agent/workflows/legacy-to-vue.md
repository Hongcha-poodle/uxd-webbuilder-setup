...
3. **코드 변환 및 생성** → `expert-legacy-to-vue` 위임:
   - 생성 코드에서 `Props` API 자동 생성은 금지합니다 (`type Props`, `defineProps`, `withDefaults(defineProps(...))` 미사용).
...
4. **[HARD] 파일 저장 전 CSS 무결성 검증**:
   - `.vue` 파일을 저장하기 **직전에** 다음 체크리스트를 반드시 수행합니다:
     1. `<template>` 내 모든 Tailwind 클래스가 누락 없이 존재하는지 확인
     2. `<style scoped>` 블록이 존재하는 경우, 모든 CSS 선언이 보존되어 있는지 확인
     3. 긴 `class` 속성(3줄 이상)이 잘림(truncation) 없이 완전한지 확인
   - 검증 실패 시 저장을 중단하고 누락된 스타일을 복원한 후 재저장합니다.

5. **[HARD] 검증 핸드오프 (필수 — 건너뛰기 절대 금지)**:
   - 저장이 완료되면 **반드시** `/component-validation` 워크플로우를 호출하여 QA 단계를 위임합니다.
   - **이 단계를 건너뛰거나 생략하는 것은 절대 금지합니다.** 사용자에게 프리뷰 URL만 제공하고 검증을 생략하는 행위를 금지합니다.
