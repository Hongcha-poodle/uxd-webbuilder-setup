...
오케스트레이터, 에이전트 규칙, preview/runtime scaffold가 업데이트되었을 때 터미널에서 아래 명령어로 내 프로젝트에 반영합니다.

> 주의:
> `pages/preview/*`, `utils/*`, `tests/*`, `nuxt.config.ts` 같은 공통 scaffold 파일은 업데이트 시 덮어써질 수 있습니다.
> 로컬 커스텀 수정이 있으면 먼저 `git diff` 확인 또는 백업 후 실행하세요.
...
| `pages/preview/*` | 기본/모바일/raw preview 라우트 |
| `utils/preview-resolver.ts` | preview 식별자와 동적 import resolver |
...
- root 컴포넌트는 기존처럼 `/preview/[ComponentName]` 형태를 그대로 사용합니다.
- 하위 폴더 컴포넌트는 갤러리에서 생성된 preview 식별자를 그대로 사용하면 열 수 있습니다. 예: `forms/LoginForm.vue` -> `/preview/forms__LoginForm`
- 목록과 상세/raw/mobile 프리뷰는 동일한 resolver를 사용합니다.
...
`npm run test`에는 workflow 문서뿐 아니라 preview resolver와 legacy dependency audit 회귀 검사도 포함됩니다.
...
- `Inter-component Dependencies`: 실제 파일 존재 여부, missing module
- `Unresolved Dependencies`: 코드 내 TODO 위치와 대응
- missing asset 또는 missing module이 있으면, 깨지는 직접 `import` 대신 placeholder 또는 조건부 렌더로 낮춰야 합니다.
