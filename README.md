...
- 주황 박스: 사용자 승인 후 `visual-diff` 워크플로우 기준으로 시각 비교 교정
- 마지막 초록 박스: 확인 링크 제공
...
아래 flowchart는 실제 실행 순서를 요약한 것입니다. `visual-diff`는 사용자 승인 시에만 실행되며, Figma 기준 이미지는 MCP `get_screenshot`으로만 확보합니다.
...
flowchart TD
    ...
    V4 --> V0
    V3 -- "통과" --> AskVD{"visual diff 진행 여부\n사용자 확인"}
    AskVD -- "승인" --> VD["visual-diff 호출\nMCP get_screenshot 기준 비교"]
    AskVD -- "보류/거부" --> V6(["프리뷰 URL 제공\n브라우저 확인"])
