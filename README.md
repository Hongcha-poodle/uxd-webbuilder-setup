...
> [!IMPORTANT]
> **매번 작업 시작 전, 반드시 로컬 서버를 실행해야 합니다.**
> 이 레포를 열 때마다 터미널에서 `npm run dev`를 실행해야 브라우저 프리뷰를 확인할 수 있습니다.
> 서버가 꺼져 있으면 `http://localhost:3000`이 열리지 않습니다.
...
설치는 한 번만 합니다. 하지만 **VS Code나 터미널을 닫았다가 다시 열면 서버도 함께 꺼집니다.**  
프리뷰를 확인하려면 프로젝트 폴더에서 아래 명령어를 다시 실행하세요.

```bash
npm run dev
```

터미널에 `localhost:3000`이 표시되면 서버가 정상 실행된 것입니다.
