...
4. **에셋 안내 및 의존성 수집**:
   - 레거시 코드에 이미지나 SVG 아이콘 참조가 있으면 `~/assets/images/`, `~/assets/icons/` 배치 기준을 안내하고, 폰트는 Pretendard CDN 웹폰트 사용 기준으로 확인합니다.
...
5. **코드 변환 실행** → `expert-legacy-to-vue` 위임:
  ...
  - **Dependencies Report**를 사용자에게 첨부합니다:
    - CSS Variables 매핑 목록
    - Inter-component Dependencies (글로벌 함수/타 컴포넌트 참조, 실제 파일 존재 여부, missing module 구분)
    - Third-party Libraries (제3자 라이브러리 및 대체 방안)
    - Assets (resolved assets / missing assets 분리, 실제 배치 경로)
  - unresolved dependency는 코드 내 TODO와 Dependencies Report 양쪽에 모두 남깁니다.
  - 실제 에셋 파일 또는 타 컴포넌트 파일이 없는 경우, 깨지는 `import`를 그대로 남기지 않고 안전한 placeholder 또는 조건부 렌더로 낮춘 뒤 `missingAssets`, `missingComponentDeps`, `unresolvedDependencies`에 기록합니다.
