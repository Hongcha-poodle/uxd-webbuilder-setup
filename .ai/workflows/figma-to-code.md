...
1. **규칙 로드**:
   ...
   - 인터랙션 구현이 필요한 컴포넌트일 경우, 먼저 `@.ai/rules/development/expert-vue-scripting.md` 인덱스를 읽고 컴포넌트 유형을 판별합니다. 이후 `@.ai/rules/development/expert-vue-scripting/patterns-forms.md`, `@.ai/rules/development/expert-vue-scripting/patterns-interactive-ui.md`, `@.ai/rules/development/expert-vue-scripting/patterns-motion-display.md`, `@.ai/rules/development/expert-vue-scripting/interop-and-extension.md` 중 필요한 모듈만 추가로 로드하여 스크립트 패턴 가이드를 적용합니다.
...
3. **코드 생성** → `expert-figma-to-vue` 위임:
   - 생성 코드에서 `Props` API는 실제 입력 계약이 명확한 경우에만 추가합니다. 시각 정보만으로 `type Props`, `defineProps`, `withDefaults(defineProps(...))`를 과도하게 추론해 생성하지 않습니다.
