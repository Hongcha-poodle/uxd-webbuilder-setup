...
3. **스타일 보존 의무 (Style Preservation Obligation)**:
   - 수정 대상이 아닌 Tailwind 클래스를 삭제하거나 변경하는 것을 **절대 금지**합니다.
   - `<style scoped>` 블록이 존재하는 경우, 교정 대상인 속성만 수정하고 나머지 CSS 선언은 그대로 유지합니다.
   - 매 iteration에서 수정 적용 후, 수정 전(`before_fix`)과 비교하여 **의도하지 않은 클래스/CSS 삭제가 없는지** 텍스트 diff로 검증합니다. 의도치 않은 삭제가 발견되면 즉시 롤백합니다.
   - **[HARD] `<style scoped>` 완전성 검증**: 매 iteration의 수정 적용 후, 다음을 반드시 확인합니다:
     1. `before_fix` 시점의 `<style scoped>` 블록 내 CSS 선언 수와 수정 후의 선언 수를 비교합니다.
     2. 수정 대상이 아닌 CSS 선언이 누락되었으면 **즉시 롤백**합니다.
     3. `<style scoped>` 블록 자체가 통째로 사라진 경우 **즉시 롤백**하고 리포트에 "CRITICAL: `<style scoped>` 블록 유실 감지"로 기록합니다.
   - **[HARD] 파일 전체 rewrite 금지**: 교정 시 파일 전체를 다시 작성하지 않습니다. 반드시 수정이 필요한 `class` 속성 또는 `<style scoped>` 내 특정 CSS 선언만 부분 수정(partial edit)합니다. 전체 rewrite는 스타일 유실의 주요 원인이므로 절대 금지합니다.
...
- 동일 카테고리(예: COLOR, TYPOGRAPHY)에 속하는 이슈가 3건 이상이고, 수정이 서로 독립적(다른 요소에 적용)인 경우, **한 번의 iteration에서 동일 카테고리 이슈를 일괄 수정**할 수 있습니다.
- 단, 배치 수정 후 회귀가 발생하면 전체를 롤백하고 개별 수정으로 전환합니다.
