...
- 변환 시점에 실제 파일 존재 여부를 확인할 수 없는 경우, 추정 `import`를 그대로 생성하지 않고 TODO와 placeholder 렌더링으로 낮춘 뒤 **Dependencies Report**의 `missingAssets`에 기록합니다.
...
- **보고 단계**:
  - **Dependencies Report**를 작성합니다:
    ...
    - **Inter-component Dependencies**: 글로벌 함수/타 컴포넌트 참조 목록, `<!-- TODO -->` 위치, unresolved dependency 목록, 실제 파일 존재 여부
    - **Third-party Libraries**: 제3자 라이브러리 사용 및 대체 방안
    - **Assets**: resolved assets / missing assets / 배치 경로
- **출력 단계**:
  - 결과물은 저장 가능한 Vue SFC 기준으로 반환합니다.
  - validation handoff metadata에는 `hasScopedStyle`, `scopedStyleDeclarationCount`, `cssVariableMappings`, `customTailwindTokens`, `arbitraryValues`, `resolvedAssets`, `missingAssets`, `missingComponentDeps`, `unresolvedDependencies`를 포함합니다.
