...
2. **정적 분석**: 타입 선언의 허술함(`any` 남용 등), 정의되지 않은 템플릿 변수, 불안정한 조건 분기, 시맨틱 구조, Tailwind 오탈자를 검열합니다.
   - 프리뷰 라우트가 실제 컴포넌트 import 경로와 동일한 resolver를 사용하는지 확인합니다.
   - Legacy 변환 산출물은 `~/assets/` import의 실제 파일 존재 여부와 `missingAssets`/`missingComponentDeps` 보고 여부를 함께 검증합니다.
   - unresolved dependency가 직접 깨지는 `import`로 남아 있으면 차단 이슈로 보고합니다.
