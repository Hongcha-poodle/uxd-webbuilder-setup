...
1. **테스트 프레임워크 준수**:
   ...
   - **Nuxt auto-import 미지원**: 테스트 환경(`vitest.config.ts`)은 Nuxt runtime이 아니므로 `ref`, `computed`, `reactive` 등 Vue composable은 반드시 명시적으로 import해야 합니다.
     ```typescript
...
### B. 경량 검증 모드 (Light Validation) — 기본 모드

`/component-validation` 워크플로우에서 기본으로 사용되는 경량 검증입니다. 유닛 테스트를 생성하지 않고 정적 분석에만 집중합니다.

1. **Lint 검사**: `.nuxt/` 디렉토리가 이미 존재하면 `nuxt prepare`를 건너뛰고 `npm run lint`를 바로 실행합니다. `.nuxt/` 디렉토리가 없는 경우에만 `npm run prepare`를 먼저 실행합니다. `quality.yaml`의 "0 lint errors" 기준을 충족해야 합니다.
2. **코드 리뷰 수준 타입 점검**: 대상 `.vue` 파일의 `<script setup>` 블록을 읽고 정의되지 않은 변수 참조, `any` 타입 남용, 템플릿 내 잠재 오류를 확인합니다.
3. **결과 보고**: 에러가 없으면 검증 완료를 보고합니다. 에러가 있으면 원본 에이전트에 피드백합니다 (최대 3회).

### C. 전체 검증 모드 (Full Validation) — 사용자 요청 시

사용자가 명시적으로 유닛 테스트 작성을 요청한 경우에 사용합니다. 경량 검증(B)의 모든 항목을 포함하며, 추가로 유닛 테스트를 작성합니다.

1. **Lint 검사**: 경량 검증 모드(B)의 1항과 동일합니다.
2. **타입 체크**: 대상 파일의 `Props`나 `ref` 구조에서 타입 선언이 `any` 등으로 허술하게 작성되지 않았는지 검열합니다.
3. **잠재 오류 발견**: 템플릿 내에서 정의되지 않은 변수를 호출하거나, `v-if` 분기에서 오류가 날 법한 렌더링 조건을 잡아냅니다.
4. **유닛 테스트 작성**: A항의 절차에 따라 `.spec.ts` 테스트 코드를 작성합니다.
5. **가정 시나리오 제안**: 테스트 코드를 만들 수 없거나 외부 환경이 필요한 경우 수동 테스트(QA) 시나리오 리스트를 작성하여 제공합니다.
