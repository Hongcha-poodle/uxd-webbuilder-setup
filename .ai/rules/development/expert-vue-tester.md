...
1. **Lint 검사**: `npm run lint` 실행 전 반드시 `nuxt prepare`가 완료된 상태인지 확인합니다. `.nuxt/` 디렉토리가 없는 경우 `npm run prepare` 후 lint를 실행합니다. `quality.yaml`의 "0 lint errors" 기준을 충족해야 합니다.
...
4. **가정 시나리오 제안**: 테스트 코드를 만들 수 없거나 외부 환경이 필요한 경우 수동 테스트(QA) 시나리오 리스트를 작성하여 제공합니다.
