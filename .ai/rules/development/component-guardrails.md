...
- **[HARD] Props 생성 금지**: AI 자동 생성 결과물에서 `type Props`, `interface Props`, `defineProps`, `withDefaults(defineProps(...))` 패턴을 **일절 추가하지 않습니다**. Props 정의는 개발팀이 직접 추가합니다.
- 이벤트 알림이 필요한 경우에만 최소 `Emits`(`defineEmits`) 중심으로 유지합니다.
...
- 컴포넌트 생성/수정 후 순서는 반드시 다음을 따릅니다:
  ...
  3. 사용자에게 `/visual-diff` 진행 여부 확인
  4. `/visual-diff` 또는 사용자 보류/거부 기록
  5. 프리뷰 URL 제공
- validation을 생략한 채 프리뷰 URL만 먼저 제공하는 것은 금지합니다.
- visual diff는 사용자 승인 후에만 실행할 수 있습니다.
- 사용자가 visual diff를 보류하거나 거부한 경우, 해당 사실을 리포트에 명시한 뒤 프리뷰 URL 제공 단계로 진행합니다.
- visual diff는 Figma MCP `get_screenshot`으로 기준 이미지를 확보할 수 없는 경우 workflow abort 조건으로 건너뜁니다.
