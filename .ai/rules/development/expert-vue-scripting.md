...
### 판별 시점 (When to Classify)

컴포넌트 유형 판별은 **코드 생성 전에 반드시 선행**되어야 합니다.

| 워크플로우 | 판별 시점 | 행동 |
|-----------|---------|------|
| **Figma → Vue** | Figma 노드 분석 완료 직후, 코드 작성 전 | 판별 결과를 기반으로 즉시 코드 생성 진행 |
| **Legacy → Vue** | JS 클래스/함수 구조 분석 직후, 변환 전 | 판별 결과를 기반으로 즉시 코드 변환 진행 |
